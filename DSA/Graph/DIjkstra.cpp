...
vector<int> dijkstra(vector<vector<int>> &vec, int n, int e, int src)
{
    ...
    // sorce node distance to 0
    dist[src] = 0;
    st.insert(make_pair(0, src)); // src insert in set

    while (!st.empty())
    {
        // get top from set(small pair always come)
        auto top = *(st.begin());
        ...
        {
            // in new distance is smaller
            if (nodeDist + neighbour.second < dist[neighbour.first])
            {
                // find if same entry in set
                auto record = st.find(make_pair(dist[neighbour.first], neighbour.first));

                // remove same entry: because it have bigger dist
                if (record != st.end())
                {
                    st.erase(record);
                }
